...
animations.to = function (window, options)
	...
	for key, value in pairs(options) do
		if key == "row" then
			animation_table[key] = helpers.createFrames("linear", vim.api.nvim_win_get_config(window)["row"], value, 10)
		elseif key == "col" then
			animation_table[key] = helpers.createFrames("linear", vim.api.nvim_win_get_config(window)["col"], value, 10)
		elseif key == "width" then
			animation_table[key] = helpers.createFrames("linear", vim.api.nvim_win_get_width(window), value, 10)
		elseif key == "height" then
			animation_table[key] = helpers.createFrames("linear", vim.api.nvim_win_get_height(window), value, 10)
		elseif key == "winblend" then
			animation_table[key] = helpers.createFrames("linear", vim.wo[window].winblend, value, 10, true)
		elseif key == "border_color" then
			animation_table[key] = helpers.createColors("linear", vim.tbl_isempty(vim.api.nvim_get_hl(0, { name = "FloatBorder_" .. window })) and vim.api.nvim_get_hl(0, { name = "FloatBorder" }).fg or vim.api.nvim_get_hl(0, { name = "FloatBorder_" .. window }).fg, value, 10)
		end
	end
