...
highlights.groups = {
	...
	["C0"] = function ()
		---+${hl}
		local buf_bg = highlights.get_property(
			"bg",
			{ "Layer1" },
			"#BFC7D7",
			"#3C3C5C"
		);
		local h_fg = highlights.get_property(
			"fg",
			{ "Comment" },
			"#9CA0B0",
			"#6C7086"
	...
	["D1"] = function ()
		local BG = highlights.get_property(
			"fg",
			{ "Layer0" },
			"#9CA0B0",
			"#6C7086"
		...
		for s, source in ipairs(sources) do
			for i = 0, 4, 1 do
				local mix = {
					lerp(source[1], BG[1], i / 4),
					lerp(source[2], BG[2], i / 4),
					lerp(source[3], BG[3], i / 4),
				};

				table.insert(_o, {
					group_name = string.format("Gradient%dN%d", s, i),
					value = {
						fg = highlights.rgb_to_hex(mix)
					}
				});
			end
		end
	...
	["D2"] = function ()
		...
		for i = 0, 9, 1 do
			...
			table.insert(_o, {
				group_name = string.format("Shadow%d", i),
				value = {
					fg = highlights.rgb_to_hex(mix)
				}
			});
